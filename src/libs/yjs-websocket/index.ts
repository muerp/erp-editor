...
import * as syncProtocol from '../yjs-protocols/sync'
import * as authProtocol from '../yjs-protocols/auth'
import * as awarenessProtocol from '../yjs-protocols/awareness'
...
messageHandlers[messageRecover] = (_t, _, provider) => {
  provider.emit('recover', [])
}
...
export class WebsocketProvider extends Observable<string> {
  ...
  _unloadHandler: () => void
  _checkInterval: NodeJS.Timeout | number
  _subDocsHandler: ({
    added,
    removed,
    loaded,
  ...
  constructor(
     ...
     * @param callback
     */
    const waitSend = (message: any, callback: ()=>void) => {
      waitForConnection(() => {
