...
const DefaultTitle: FC<TitleComponentProps> = ({ attributes, children }) => {
  return <div className='docx-title' {...attributes}>{children}</div>
}
...
export const withTitle = <T extends Editable>(editor: T, options: TitleOptions = {}) => {
  ...
  let titleString: string | undefined = undefined;
  titleEditor.normalizeNode = entry => {
    ...
    if (Editor.isEditor(node)) {
      ...
      if (titleString === undefined) {
        titleString = Node.string(node);
      } else {
        const cur = Node.string(node);
        if (cur !== titleString) {
          titleString = cur;
          options.onChange && options.onChange(Node.string(node));
        }
  ...
  Placeholder.subscribe(
    editor,
    ([node]) => {
      if (Title.isTitle(node)) return () => options.placeholder ?? '请提炼模型'
    },
    true,
  )
